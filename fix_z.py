...
if __name__ == '__main__':
    ...
    train = [False]
    z = sess.run(vae.z, {vae.x: test_X, vae.y: test_y, vae.train: train})
    #first = test_y[0][10]
    #step = test_y[0][10]/10
    #new_y = test_y[0].copy()
    ...
    output_X = sess.run(vae.pi_out, {vae.y: new_y, vae.z_in: z, vae.train: train})

    fig = plt.figure(figsize=(20, 20))

    imshow10(test_X, output_X, fig)

    plt.savefig('fix_z.png')
